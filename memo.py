...
# fingers = detector.fingersUp() 부분 추가
print(fingers)
[1, 0, 0, 0, 0] -> 손을 모두 오무리면 앞자리가 1 , 나머지가 0으로 들어옴
[0, 1, 1, 1, 1] -> 손을 활짝 펴면 앞자리가 0 나머지가 모두 1로됨. (손가락을 전부세워서)
[1, 1, 1, 1, 1] -> 숫자 4를 손으로 표현하면 이런식이됨 (엄지 제외 모두폄)
[1, 1, 0, 0, 0] -> 제일 앞자리 1개를 제외하고, 순서대로 검지/중지/약지/소지